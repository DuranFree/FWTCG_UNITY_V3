...
namespace McpUnity.Unity
{
    ...
    public class McpUnitySocketHandler : WebSocketBehavior
    {
        ...
        protected override void OnOpen()
        {
            ...
            // Enforce MaxConnections limit: close oldest excess connections.
            int maxConn = McpUnitySettings.Instance.MaxConnections;
            if (maxConn > 0 && _server.Clients.Count > maxConn)
            {
                // Collect all session IDs except the one just connected (current ID)
                var excessIds = _server.Clients.Keys
                    .Where(k => k != ID)
                    .Take(_server.Clients.Count - maxConn)
                    .ToList();

                foreach (var excessId in excessIds)
                {
                    _server.Clients.TryRemove(excessId, out _);
                    try
                    {
                        Sessions.CloseSession(excessId, CloseStatusCode.Normal, "Max connections reached — oldest session evicted");
                    }
                    catch (Exception ex)
                    {
                        McpLogger.LogWarning($"Error closing excess session {excessId}: {ex.Message}");
                    }
                }
                McpLogger.LogInfo($"Max connections ({maxConn}) reached. Evicted {excessIds.Count} oldest session(s).");
            }
