...
- 收到任何视觉效果需求（发光/Bloom/模糊/后处理）前，必须按顺序决策：① 引擎原生（URP Post Processing / Shader Graph / VFX Graph）→ ② 材质/Shader 模拟 → ③ 手动代码模拟；禁止跳过前两步直接手动模拟
- 收到任何视觉效果需求前必须先确认渲染管线（URP/HDRP/Built-in）和 Canvas 渲染模式（Overlay/Camera/World）；Canvas Overlay 下 URP 后处理完全无效，需改为 Screen Space - Camera
