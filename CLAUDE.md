...
**⚠️ MCP / 脚本修改后必须立即保存（统一兜底，不可跳过）：**

任何修改完成后 → 立即调用 `execute_menu_item("File/Save Project")`。

它内部会先存场景再存资产（等价 `save_scene` + `AssetDatabase.SaveAssets()`），一条命令兜底全部 dirty 状态，不需要纠结改的是场景还是资产。

目的：防止编辑器处于 dirty 状态，避免后续 MCP 操作（尤其 run_tests / Play Mode 切换）触发 "Save Scene?" / "Save Assets?" 弹窗导致 MCP 超时。**每次修改后都要执行，不得批量延后。**

注意：
- Play Mode 中无法保存资产，必须在 EditMode 下调
- `FWTCG/Build Game Scene`（SceneBuilder）内部自带保存，调用后无需额外 Save Project
...
**判断原则：有固定终点用 DOTween；目标持续变化用 Update/LateUpdate Lerp。**

- ✅ 用 DOTween：一次性动画（出牌/翻转/攻击/UI进退场/淡入淡出/数字跳动/Sequence）
- ✅ 用 Update/LateUpdate Lerp：目标每帧可能变化（手牌扇形展开、悬停跟随、拖拽预览）
- ❌ 禁止：用 DOTween 做持续追踪（每帧 kill + restart tween，性能差且不稳定）
- ❌ 禁止：用协程手写动画插值循环（while + yield return null + Lerp → 改用 DOTween）
- ✅ 允许：流程控制协程（WaitForSeconds / WaitUntil / 游戏逻辑等待），但协程内部的动画仍用 DOTween
...
- 子节点需要位移/旋转/缩放动画时，容器禁止挂 LayoutGroup；静态排列（菜单/背包/按钮组）才用 LayoutGroup
- **`async void` 禁止用在 MonoBehaviour 上**：对象销毁后回调照跑，会访问已销毁组件崩溃。一律改用 `async Task` / `async UniTask` + `CancellationToken`，`OnDisable`/`OnDestroy` 中 `_cts.Cancel()`
- **热路径禁止 `Instantiate` / `Destroy`**：用 `ObjectPool<T>`（Unity 2021+）代替，子弹/特效/伤害数字等高频对象必须池化
